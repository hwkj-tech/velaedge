...
- `crates/edge-runtime`: deterministic edge execution, Modbus RTU and DL/T 645-2007 serial collection, RocksDB config state, algorithm DSL execution, scheduling, EdgeLink connectivity, and MQTT uplink.
...
can also create/update/publish, and `admin` additionally controls deletion and edge access-token
generation. `GET /api/auth/me` returns the authenticated subject and effective role. EdgeLink runtime
...
On Unix, `cargo test -p edge-runtime --test serial_pty` exercises the production
`TokioSerialBusFactory` against an OS pseudo terminal and verifies a CRC-protected Modbus request
and response. This covers the real async serial file-descriptor path; physical RS-485 wiring,
adapter direction control, and non-zero baud-rate ioctls remain site acceptance checks.
