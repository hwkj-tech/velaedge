...
<body>
  ...
  <main>
    ...
    <div class="layout"><div>
      ...
      <section class="section"><h2>MQTT 传输 <small id="mqtt-summary">会话未建立</small></h2><div class="facts">
      ...
      <section class="section"><h2>控制连接</h2><div class="facts">
        ...
        <div class="fact"><span>MQTT 在线会话</span><strong id="mqtt-connected">--</strong></div><div class="fact"><span>MQTT 会话代次</span><strong id="mqtt-generation">--</strong></div>
...
function mqttQueueError(error){const value=String(error||"").toLowerCase();return value.includes("mqtt outbox")||value.includes("persist mqtt")}
function renderMqttSinks(rows,queueError){$("mqtt-sinks").replaceChildren(...(rows.length?rows.map(row=>{const tr=document.createElement("tr"),state=!row.connected?"会话断开":row.last_error?"发布异常":queueError?"会话在线 · 队列异常":"会话在线";const values=[row.sink_id,row.broker,state,`${row.publish_success_count} / ${row.publish_failure_count}`,`${row.average_ack_latency_ms} / ${row.last_ack_latency_ms??"--"} ms`,row.last_topic||"--",when(row.last_publish_at)];values.forEach((v,i)=>{const td=document.createElement("td");td.textContent=v;if(i===2)td.className=row.connected&&!row.last_error&&!queueError?"ok-text":"error";if(i===5){td.classList.add("topic");td.title=row.last_topic||""}tr.append(td)});return tr}):[Object.assign(document.createElement("tr"),{innerHTML:'<td colspan="7" class="empty">暂无 MQTT 输出运行数据</td>'})]))}
async function refresh(){
  try{
    ...
    const configured=q.configured_sink_count??0,connected=q.connected_sink_count??0,queueError=mqttQueueError(a.last_error),mqttSummary=!configured?"未配置 MQTT 输出":!connected?`0/${configured} 会话在线`:queueError?`${connected}/${configured} 会话在线 · 本地队列异常`:`${connected}/${configured} 会话在线`;
    text("mqtt-connected",`${connected} / ${configured}`);text("mqtt-generation",q.connection_generation??0);text("mqtt-summary",mqttSummary);$("mqtt-summary").className=!configured?"":connected===configured&&!queueError?"ok-text":"error";text("mqtt-published",q.publish_success_count??0);text("mqtt-failed",q.publish_failure_count??0);text("mqtt-bytes",bytes(q.published_bytes));text("mqtt-pending",m?.local_store?.buffered_records??0);renderMqttSinks(q.sinks||[],queueError);
...
mod tests {
    ...
    fn health_page_exposes_truthful_protocol_maturity() {
        ...
        assert!(RUNTIME_HEALTH_PAGE.contains("MQTT 在线会话"));
        assert!(RUNTIME_HEALTH_PAGE.contains("会话在线 · 队列异常"));
        assert!(RUNTIME_HEALTH_PAGE.contains("mqttQueueError"));
    }
